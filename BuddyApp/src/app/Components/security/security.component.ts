...
export class SecurityComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.firstFormGroup.controls['password'].valueChanges.subscribe(value => {
      if (value === '') {
    ...
    this.firstFormGroup.controls['passwordC'].valueChanges.subscribe(value => {
      if (value === '') {
  ...
  onSubmit(e: any) {
    ...
    if (this.firstFormGroup.valid) {
      ...
      let customer = {
        oldPassword: customerForm.oldPassword,
        appUser: {
          id: this.sharedService.getUserIdByLS(),
          password: customerForm.password
        }
      }
      this.http.put<any>(this.sharedService.publicUrl + 'app_user/reset_password/' + customer.appUser.id, customer).subscribe(result => {
